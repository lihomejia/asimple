								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0" class="dataList">
						            ...
						            <c:forEach items="${datas}" var="data" varStatus="status">
						              	<tr>
							                ...
							                <td>${data.__disp.outuserId}</td>
							                <td>${data.__disp.outdate}</td>
							                <td>${data.__disp.status }</td>
							                <td>
							                	<c:if test="${data.status == 0 }">
							                		<a class=linkStyle href="<%=basePath%>manure/outstock/approve.html?id=${data.id}" onclick="return confirm('确定批准吗?')">批准</a>
													&nbsp;|&nbsp;
							                		<a class=linkStyle href="<%=basePath%>manure/outstock/edit.html?id=${data.id}">修改</a>
													&nbsp;|&nbsp;
							                		<a class=linkStyle href="<%=basePath%>manure/outstock/delete.html?id=${data.id}" onclick="return confirm('确定删除吗?')">删除</a>
							                	</c:if>
							                	<c:if test="${data.status == 2}">
							                		<a class=linkStyle href="<%=basePath%>manure/outstock/disp.html?id=${data.id}">查看</a>
							                		&nbsp;|&nbsp;
							                		<a class=linkStyle href="<%=basePath%>manure/outstock/nullify.html?id=${data.id}" onclick="return confirm('确定作废吗?')">作废</a>
							                	</c:if>
