...
<body>
	<form id="form1" method="post">
		<table width="100%">
										...
										<tr>
											...
											<td>
												<c:if test="${data.status == 0}">
													<a class=linkStyle href="<%=basePath%>disinfectant/resource/approve.html?type=${type}&id=${data.id}" onclick="return confirm('确定批准吗?');">批准</a>
													&nbsp;|&nbsp;
													<a class=linkStyle href="<%=basePath%>disinfectant/resource/edit.html?type=${type}&id=${data.id}">修改</a>
													&nbsp;|&nbsp;
													<a class=linkStyle href="<%=basePath%>disinfectant/resource/delete.html?type=${type}&id=${data.id}" onclick="return confirm('确定删除吗?');">删除</a>
												</c:if>
												<c:if test="${data.status == 2}">
													<a class=linkStyle href="<%=basePath%>disinfectant/resource/nullify.html?type=${type}&id=${data.id}" onclick="return confirm('确定作废吗?');">作废</a>
													&nbsp;|&nbsp;
													<a class=linkStyle href="<%=basePath%>disinfectant/resource/disp.html?type=${type}&id=${data.id}">查看</a>
												</c:if>
											</td> 
										</tr>
									</c:forEach>
			...
			<tr>
				<td style="text-align:right">
					<input type="button" class="btnStyle" value="&nbsp;新&nbsp;建&nbsp;" onclick="window.location.href='<%=basePath%>disinfectant/resource/add.html?type=${type}';"/>
				</td>
			</tr>
		</table>
	</form>
