    ...
    <script type="text/javascript">
        ...
        function doOutStock(){
        	if(formCheck()) {
        		$.ajax({
				   type: "POST",
				   url: "<%=basePath%>/pesticide/outstock/checkOutStock.html",
...
<body>
	<form id="form1" name="form1" method="post" action="<c:url value='/pesticide/outstock/save.html'/>">
								      	...
								      	<td colspan="4" align="center" class=forumrow>
								        	<c:if test="${_action == 'add'}">
								      		...
								      		<c:if test="${_action == 'edit' || _action == 'disp'}">
								      			<input type="button" class="btnStyle" value="返&nbsp;回" onclick="window.location.href='<c:url value="/pesticide/outstock/list.html"/>'"/>
								      		</c:if>
								        </td>
								    </tr>
								</table>
